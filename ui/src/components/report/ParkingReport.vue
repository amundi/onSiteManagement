<template>
  <div ref="main" class="graph parking flex justify-center" :class="[hue]" :id="name + '-parking'"></div>
</template>
  ...
  methods: {
    updateView(startDate, values) {
      ...
      if (startDate && values) {
        ...
        // Empty and setup svg -------------------------------------------
        d3.select("#" + vm.name + "-parking")
          .selectAll("svg")
          .remove();

        vm.svg = d3
          .select("#" + vm.name + "-parking")
          .append("svg")
        ...
        for (let date of dates) {
          ...
          for (let val of values) {
            if (date == val.date) {
              ...
              // Arrivals ------------------------------------
              vm.addRect(10, h, 20 * unit, val["car"], "def");
              vm.addRect(17, h, 10 * unit, val["car_u"], "usage");
              vm.addRect(24, h, 20 * unit, val["moto"], "def");
              vm.addRect(31, h, 10 * unit, val["moto_u"], "usage");
              vm.addRect(38, h, 20 * unit, val["bike"], "def");
              vm.addRect(45, h, 10 * unit, val["bike_u"], "usage");
